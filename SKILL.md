...
4. Ensure two groups exist: `Issues` and `Pull Requests` (`create_group`).
...
5. Write the state file now, before any item writes. If the run dies
...
The default `Person` column is **not** reused either. This skill assigns
nobody: a GitHub issue has no opinion about who should own the monday row, and
guessing puts work in somebody's "My Work" queue on the strength of it. Rows
arrive unowned and a human assigns them.
...
```bash
scripts/resolve-authors.py --github issues.json \
  [--details pulls.json] [--reviews reviews.json] [--fallback LOGIN]
```

The `Author` column names a **person, never a bot**. Most items resolve for
free — work pushed through a harness is authored by the developer's own GitHub
account, and the tool appears only as a `Co-Authored-By` trailer. Only genuine
third-party apps (dependabot and friends) open items under their own identity.

For those, and *only* those, fetch `repos/OWNER/REPO/pulls/<n>` and
`.../pulls/<n>/reviews` keyed by number into `pulls.json` / `reviews.json`; the
resolver then tries the merger, whoever enabled auto-merge, and the first
approver. `needs_detail()` tells you which items justify the extra call — it is
usually a small fraction, so do not fetch details for the whole repo.

Any item it reports as **unresolved** cannot be written: there is no human to
name. Ask for `options.automationAuthor` and re-run rather than falling back to
the bot's own login.
...
**If `resolve-authors.py` reported any `unresolved` item**, the run cannot
proceed for those: the Author column may not contain a bot, and GitHub offered
no human. Report the count and ask once:

> 7 of 8 PRs were opened by automation (`dependabot`) and nobody has merged,
> approved, or auto-merged them, so GitHub has no human to attribute them to.
> Which GitHub login is accountable for automation here? You can also
> **exclude** that author's items from the board entirely.

Record the answer in state so it is not asked again — `options.automationAuthor`
for attribution, `options.excludeAuthors` for exclusion. Never suggest a
specific login; read it from the user.

Items whose author resolved from GitHub need no question — the human who merged
or approved is already the right answer.
